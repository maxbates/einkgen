...
└── lambdas/
    ...
    ├── read_api.py             GET /queue, /generated, /history, /status (newest single device), /devices (all devices, newest-first; capped at MAX_DEVICES_LIMIT=200)
...
| "Images are too similar" / "Mode-collapsed on the same landmarks" / "Make expansions more diverse" | Cron-side only — admin **Now**/**Run** and email submissions pass literal prompts, no `expand_topic` involved. Two knobs: (a) grow the per-axis bags in [src/einkgen/core/angles.py](src/einkgen/core/angles.py) (region/era/light/scale/mood) — bigger = more variety, no cost change; (b) bump `RECENT_PROMPT_AVOID_LIMIT` in [src/einkgen/lambdas/generator.py](src/einkgen/lambdas/generator.py) (default 30) to lengthen the no-repeat memory. After tweaking either, run `scripts/sample_expansions.py` to eyeball variety before redeploying. |
...
| "Device is stuck on the same image" / "Got a `generated-queue-empty` page" / "Buffer-empty alarm fired" | Since 0.6.5.0 the generator emits a literal `BUFFER_EMPTY_AFTER_REFILL` log token whenever a cron tick finishes with generated-queue depth = 0. The CDK alarm `einkgen-<env>-generated-queue-empty` pages via the existing alarm SNS topic after two consecutive empty ticks. Common causes: (a) the operator emptied `config/prompt_library.txt` and `expand_topic` had nothing to expand — `einkgen prompts reset` restores defaults; (b) `expand_topic` is failing (OpenAI outage / bad `EINKGEN_TEXT_MODEL`) — `aws logs tail /aws/lambda/einkgen-generator --follow` will show the upstream error. The device keeps drawing the last buffered frame; admin **Show this now** on any history tile is a manual workaround until the buffer refills. |
