...
### 1.7. Pick a render cadence (optional, but think about it now)

Every cron tick produces one OpenAI image-generation call. The cadence
controls both the OpenAI bill and the Inkplate's battery life. A single
knob (`einkgenPollIntervalSeconds` in `infra/cdk.json`) drives both
the cron rate and the device-poll hint, so they can't drift.

| Cadence | Seconds | Renders / day | Approx. OpenAI bill | Approx. battery life |
| --- | --- | --- | --- | --- |
| 15 minutes | `900` | 96 | ~$115/mo | ~10–12 weeks |
| **30 minutes (default)** | `1800` | 48 | ~$55/mo | ~3–4 months |
| 1 hour | `3600` | 24 | ~$30/mo | ~6–9 months |
| 2 hours | `7200` | 12 | ~$15/mo | ~1 year |
| 3 hours | `10800` | 8 | ~$10/mo | ~1.5 years |

Battery numbers assume the stock 3 Ah cell; OpenAI numbers assume
`gpt-image-2` medium at ~$0.04/render.

If you want the default (30 min): do nothing, skip to §1.8.

If you want a different cadence ≤ 1 hour (3600 s): edit
`infra/cdk.json` now and set
`"einkgenPollIntervalSeconds": "<seconds>"`. No firmware change needed
— the device's `SLEEP_MAX_SECONDS = 1 h` is a cap on long sleeps, not
a target, so any value ≤ 3600 is obeyed directly.

If you want a cadence > 1 hour (e.g. 7200 for 2 h or 10800 for 3 h):
edit `infra/cdk.json` AND raise `SLEEP_MAX_SECONDS` +
`SLEEP_FALLBACK_SECONDS` in `firmware/inkplate10/inkplate10.ino` to
match before re-flashing the device. Without the firmware bump the
device just polls every hour anyway and the server's longer hint is
silently clamped.

You can change this later — just edit `cdk.json` and redeploy. See
§3.12.

### 1.8. (One-time) CDK bootstrap
...
> - Render cadence: `default` (30 min) — or another value you picked in
>   §1.7 and already wrote into `infra/cdk.json`. If you want > 1 h you
>   also re-flash the firmware (`SLEEP_MAX_SECONDS`); ≤ 1 h is
>   server-only.
>
...
### 3.12. Change the render / poll cadence later

The single knob `einkgenPollIntervalSeconds` in
[infra/cdk.json](infra/cdk.json) drives **both** the EventBridge cron
rate that fires the generator **and** the manifest's `next_check_after`
hint that tells the Inkplate when to wake. Default: `1800` (30 min).
See the table in §1.7 for cost / battery trade-offs.

**To change** (≤ 1 hour — server-only):

1. Edit `infra/cdk.json`, set `"einkgenPollIntervalSeconds": "<seconds>"`.
2. Redeploy: `AWS_PROFILE=einkgen ./infra/scripts/deploy.sh`.
3. Add a `CHANGELOG.md` entry noting the new cadence and the rough
   monthly cost.

The new cron rate takes effect at the EventBridge rule update;
manifests already on S3 keep their old `next_check_after` hint until
the next publish refreshes them.

**To change** (> 1 hour, e.g. 7200 for 2 h):

1. Edit `infra/cdk.json` as above.
2. Edit `firmware/inkplate10/inkplate10.ino` and raise both
   `SLEEP_MAX_SECONDS` and `SLEEP_FALLBACK_SECONDS` to match. They live
   near the top of the file under `// ----- Tunables -----`.
3. Re-flash the device.
4. Redeploy + `CHANGELOG.md` entry.

Without step 2 the firmware silently clamps to its 1 h cap; the
server's longer hint is ignored and the device polls every hour
anyway. Going the other direction (server hint shorter than the
firmware cap) is always safe — the firmware honours the shorter
value.

Constraints enforced at `cdk synth`: `einkgenPollIntervalSeconds`
must be a positive integer, at least 60, and a multiple of 60
(EventBridge `rate()` only accepts whole-minute schedules).

> **Agent note:** when a human asks to change the cadence, this is
> the only knob — don't reach for hardcoded `Duration.minutes(...)`
> values in [infra/lib/lambdas.ts](infra/lib/lambdas.ts); the rate
> is computed from the prop. And always bump `CHANGELOG.md` with
